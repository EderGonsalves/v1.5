...
export const StepConnections = () => {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (data.auth?.institutionId) {
      router.push(`/dashboard?institutionId=${data.auth.institutionId}`);
    }
  };
  ...
  const handleWhatsAppConnect = () => {
    ...
    console.log("Redirecionando para OAuth WhatsApp:", oauthUrl);
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          Conecte os canais de comunicação que o agente utilizará para atender
          seus clientes. Você pode configurar isso agora ou depois.
        </p>
      </div>
            ...
            <div className="flex-1 space-y-2">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Conecte sua conta do WhatsApp Business para o agente
      ...
      <StepActions submitLabel="Continuar" isSubmitting={isConnecting} />
    </form>
  );
};
